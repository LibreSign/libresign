 ...
 *   We have to verify against DPO GetMobilePaymentOptions per country.
	...
	public function resolve(string $region, string $localNumber): array
	{
		...
		if ($localNumber === '') {
			return $this->buildResult(
				...
				$region,
				'Empty or invalid local number after normalization'
			);
		}
		...
		if ($regionMap === null) {
			return $this->buildResult(null, ResolutionConfidence::UNKNOWN, $region,
		...
		if (count($matches) > 1) {
			...
			return $this->buildResult(null, ResolutionConfidence::AMBIGUOUS, $region,
		...
		if ($match['ambiguous']) {
			return $this->buildResult($match['mno'], ResolutionConfidence::AMBIGUOUS, $region,
				$match['note']
			);
		}

		// Clean single match
		return $this->buildResult($match['mno'], ResolutionConfidence::HIGH, $region, null);
	}
