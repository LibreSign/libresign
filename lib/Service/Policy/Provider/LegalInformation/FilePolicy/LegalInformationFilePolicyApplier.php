...
class LegalInformationFilePolicyApplier implements IFilePolicyApplier {
	...
	private function getOverrides(array $data): array {
		if (isset($data['policyOverrides']) && is_array($data['policyOverrides']) && array_key_exists(LegalInformationPolicy::KEY, $data['policyOverrides'])) {
			return [LegalInformationPolicy::KEY => $data['policyOverrides'][LegalInformationPolicy::KEY]];
		}
	...
	private function assertOverrideAllowed(array $requestOverrides, ResolvedPolicy $resolvedPolicy): void {
		...
		throw new LibresignException($this->l10n->t('Legal information override is blocked by %s.', [$blockedBy]), 422);
	}
