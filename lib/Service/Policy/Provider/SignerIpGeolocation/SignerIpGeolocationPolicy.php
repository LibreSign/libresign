...
final class SignerIpGeolocationPolicy implements IPolicyDefinitionProvider {
	public const KEY = 'signer_ip_geolocation';
	public const SYSTEM_APP_CONFIG_KEY = 'signer_ip_geolocation';
	...
	public function get(string|\BackedEnum $policyKey): IPolicyDefinition {
		return match (PolicyKeyNormalizer::normalize($policyKey)) {
			self::KEY => new PolicySpec(
				key: self::KEY,
				defaultSystemValue: SignerIpGeolocationPolicyValue::defaults(),
				allowedValues: static fn (): array => [],
				normalizer: static fn (mixed $rawValue): array => SignerIpGeolocationPolicyValue::normalize($rawValue),
				validator: static function (mixed $value): void {
					...
					if (SignerIpGeolocationMode::tryFrom((string)$value['mode']) === null) {
