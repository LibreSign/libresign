...
const TsaRuleEditor = defineComponent({
	name: 'TsaRuleEditor',
	props: ['modelValue', 'editorScope', 'editorMode'],
	emits: ['update:modelValue'],
	template: '<div class="tsa-rule-editor-stub">TSA editor</div>',
})

const CrlValidationRuleEditor = defineComponent({
	name: 'CrlValidationRuleEditor',
	props: ['modelValue', 'editorScope', 'editorMode'],
	emits: ['update:modelValue'],
	template: '<div class="crl-validation-rule-editor-stub">CRL validation editor</div>',
})
...
describe('Preferences view', () => {
	...
	it('does not render TSA preference when an administrator assigned it at user scope', async () => {
		getPolicyMock.mockImplementation((key: string) => {
			if (key === 'tsa_settings') {
		...
		expect(wrapper.text()).not.toContain('Timestamp Authority')
		expect(wrapper.vm.preferenceEntries.some((entry: { definition: { key: string } }) => entry.definition.key === 'tsa_settings')).toBe(false)
		expect(wrapper.findComponent({ name: 'TsaRuleEditor' }).exists()).toBe(false)
	})

	it('does not render external CRL validation preference when an administrator assigned it at user scope', async () => {
		getPolicyMock.mockImplementation((key: string) => {
		...
		expect(wrapper.text()).not.toContain('External CRL validation')
		expect(wrapper.vm.preferenceEntries.some((entry: { definition: { key: string } }) => entry.definition.key === 'crl_external_validation_enabled')).toBe(false)
		expect(wrapper.findComponent({ name: 'CrlValidationRuleEditor' }).exists()).toBe(false)
	})

	it('does not render request expiration preference when an administrator assigned it at user scope', async () => {
		getPolicyMock.mockImplementation((key: string) => {
		...
		expect(wrapper.text()).not.toContain('Request expiration')
	})
