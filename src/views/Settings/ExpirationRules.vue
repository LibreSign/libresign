	...
	methods: {
		...
		async getMaximumValidity() {
			const response = await axios.get(
				generateOcsUrl('/apps/provisioning_api/api/v1', 2) + '/config/apps/libresign/maximum_validity', {},
			)
		...
		async getRenewalInterval() {
			const response = await axios.get(
				generateOcsUrl('/apps/provisioning_api/api/v1', 2) + '/config/apps/libresign/renewal_interval', {},
			)
