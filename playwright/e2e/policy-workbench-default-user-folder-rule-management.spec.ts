...
test('default_user_folder allows creating and persisting a system rule from workbench UI', async ({ page }) => {
	await bootstrapLibreSignAdmin(page)
	await page.goto('./settings/admin/libresign')

	const settingCard = await ensureCatalogSettingCardVisible(page, /Customize default account folder/i, 'folder')
	await settingCard.click()

	const dialog = page.getByRole('dialog').filter({ hasText: /Customize default account folder/i }).first()
	...
	const folderInput = createDialog.getByLabel(/Folder name/i).first()
	if (!await folderInput.isVisible().catch(() => false)) {
		const enableSwitch = createDialog.locator('.checkbox-radio-switch')
			.filter({ hasText: /Customize default account folder/i })
			.first()
		if (await enableSwitch.isVisible().catch(() => false)) {
			await enableSwitch.locator('.checkbox-radio-switch__content').first().click()
		}
	}
	await expect(folderInput).toBeVisible({ timeout: 10000 })
	await folderInput.fill('Policy Folder')
	await folderInput.blur().catch(() => {})

	const saveResponse = page.waitForResponse((response) => {
		return ['POST', 'PUT', 'PATCH'].includes(response.request().method())
			&& response.url().includes('/apps/libresign/api/v1/policies/system/default_user_folder')
	})
	...
	const reopenedCard = await ensureCatalogSettingCardVisible(page, /Customize default account folder/i, 'folder')
	await reopenedCard.click()
	const reopenedDialog = page.getByRole('dialog').filter({ hasText: /Customize default account folder/i }).first()
