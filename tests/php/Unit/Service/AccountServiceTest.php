...
use OCA\Libresign\Handler\CertificateEngine\ICertificateEngine;
...
final class AccountServiceTest extends \OCA\Libresign\Tests\Unit\TestCase {
	...
	public function testGetFileByUuidWithSuccess(): void {
		...
		$file = $this->createMock(DbFile::class);
	...
	public function testCreateToSignWithSuccessAndEmailAndSignPassword(): void {
		...
		$entity = $this->createMock(IdentifyMethod::class);
	...
	public function testGetConfigDecodesJsonConfigs(): void {
		...
		$this->identityDocumentValidator->method('userCanApproveValidationDocuments')->with($user, false)->willReturn(true);
	...
	public function testSaveVisibleElements(): void {
		...
		$this->assertTrue($userElement1->getStarred());
		$this->assertFalse($userElement2->getStarred());
	}
	...
	public function testSaveVisibleElementUpdateFileAndStarred(): void {
		...
		$this->userElementMapper->expects($this->once())
			...
			->willReturnCallback(function (UserElement $elem) {
				$this->assertTrue($elem->getStarred());
				return $elem;
			});
	...
	public function testGetFileRawWithUrlSuccess(): void {
		...
		$this->fileInputValidator->expects($this->once())
			...
			->with('fake_png_data', FileInputValidator::TYPE_VISIBLE_ELEMENT_USER);
	...
	#[DataProvider('provideInvalidFileUrls')]
	public function testGetFileRawWithInvalidUrlThrows(
		string $url,
		?string $contentType,
		?string $body,
		...
		$this->folderService->method('getFolder')->willReturn($rootFolder);
		$sessionFolder = $this->createMock(Folder::class);
		$rootFolder->method('newFolder')->with('session123')->willReturn($sessionFolder);
	...
	public static function provideInvalidFileUrls(): array {
		return [
			'invalid url format' => ['invalid-url', null, null, 'Invalid URL file'],
			'non png content type' => ['https://example.com/image.jpg', 'image/jpeg', null, 'Visible element file must be png.'],
			'empty response body' => ['https://example.com/image.png', 'image/png', '', 'Empty file'],
		];
	}
