Feature: page/validate
  ...
  Scenario: PDF endpoint returns controlled 404 when source file node is missing
    ...
    And fetch field "(SIGN_REQUEST_UUID)ocs.data.data.0.signers.0.sign_request_uuid" from previous JSON response
    And as user "admin"
    And I force sign request "<SIGN_REQUEST_UUID>" file node id to 999999999
    When as user "signer1"
    And sending "get" to "/apps/libresign/pdf/<SIGN_REQUEST_UUID>"
