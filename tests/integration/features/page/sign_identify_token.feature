Feature: page/sign_identify_token
  Background: Create users
    ...
    And sending "post" to "/apps/libresign/api/0.1/sign/register"
      ...
      | users | [{"uid":"signer1","identify":"token"}] |
      | name | document |
  ...
  Scenario: Open sign file with all data valid
    ...
    Then the response should contain the initial state "libresign-config" with the following values:
      | key | value |
      | action | 250 |
      | settings | {"identificationDocumentsFlow":false,"certificateOk":false,"hasSignatureFile":false,"phoneNumber":"","identify":"token","signMethod":"password","isApprover":false} |
      | user | {"name":""} |
