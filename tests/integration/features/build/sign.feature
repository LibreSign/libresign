Feature: sign
  Scenario: Sign setup proccess with success
    ...
    And run the command "libresign:developer:sign-setup --privateKey=apps-extra/libresign/build/tools/certificates/local/libresign.key --certificate=apps-extra/libresign/build/tools/certificates/local/libresign.crt" with result code 0
    ...
    And run the bash command "php -r \"if (count(glob('../../appinfo/*.json')) !== 10) {echo count(glob('../../appinfo/*.json'));exit(1);}\"" with result code 0
