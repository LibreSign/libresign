Feature: sign/signer_device_geolocation
  Scenario: Required geolocation is enforced server-side and stored independently of collect_metadata
    ...
    And sending "post" to ocs "/apps/libresign/api/v1/policies/system/signer_device_geolocation"
      | value | {"mode":"required"} |
    And the response should have a status code 200
    ...
    Then the response should be a JSON array with the following mandatory values
      ...
      | (jq).ocs.data.data[0].signers[0].metadata.deviceGeolocationRequirement    | required |
    ...
    When sending "post" to ocs "/apps/libresign/api/v1/sign/uuid/<SIGN_REQUEST_UUID>"
      | method | clickToSign |
      | deviceGeolocation | {"status":"collected","latitude":-23.5505,"longitude":-46.6333,"accuracy":25,"timestamp":1700000000000} |
    ...
    Then the response should be a JSON array with the following mandatory values
      | key                                                                     | value     |
      | (jq).ocs.data.data[0].signers[0].metadata.geolocation.device.status      | collected |
      | (jq).ocs.data.data[0].signers[0].metadata.geolocation.device.latitude   | -23.5505  |
  ...
  Scenario: Requester may require geolocation for selected signers when policy is optional
    ...
    When sending "post" to ocs "/apps/libresign/api/v1/request-signature"
      | file | {"base64":"<SMALL_VALID_PDF_BASE64>"} |
      | signers | [{"displayName":"Required signer","deviceGeolocationRequired":true,"identifyMethods":[{"method":"account","value":"signer1"}]},{"displayName":"Optional signer","deviceGeolocationRequired":false,"identifyMethods":[{"method":"account","value":"signer2"}]}] |
    ...
    Then the response should be a JSON array with the following mandatory values
      | key                                                                  | value    |
      | (jq).ocs.data.data[0].signers[0].metadata.deviceGeolocationRequirement     | required |
      | (jq).ocs.data.data[0].signers[1].metadata.deviceGeolocationRequirement     | disabled |
    And as user "signer1"
    And sending "get" to ocs "/apps/libresign/api/v1/file/list?details=1"
    And fetch field "(SIGN_REQUEST_UUID)ocs.data.data.0.signers.0.sign_request_uuid" from previous JSON response
    When sending "post" to ocs "/apps/libresign/api/v1/sign/uuid/<SIGN_REQUEST_UUID>"
