Feature: request-signature
  ...
  Scenario: Sign file
    ...
    And run the command "config:app:set libresign add_footer --value=1 --type=integer" with result code 0
    And run the command "config:app:set libresign write_qrcode_on_footer --value=1 --type=integer" with result code 0
    And sending "post" to ocs "/apps/provisioning_api/api/v1/config/apps/libresign/identify_methods"
