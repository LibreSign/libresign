Feature: account/signature
  Scenario: Create root certificate with OpenSSL engine using API
    ...
    And the response should be a JSON array with the following mandatory values
      ...
      | (jq).ocs.data.rootCert.names[4].value         | (jq)any(.[]; test("^libresign-ca-id:[a-z0-9]{10}(-ca:g[0-9]+-[oc])?$")) |
  ...
  Scenario: Create pfx with success using CFSSL
    ...
    Then the response should be a JSON array with the following mandatory values
      ...
      | (jq).ocs.data                             | (jq).name \|test("/OU=Organization Unit, libresign-ca-id:[a-z0-9]{10}(-ca:g[0-9]+-[oc])?") |
      ...
      | (jq).ocs.data.issuer.OU                   | (jq) .[1] \|test("^libresign-ca-id:[a-z0-9]{10}(-ca:g[0-9]+-[oc])?$")                      |
  ...
  Scenario: Create pfx with success using OpenSSL
    ...
    Then the response should be a JSON array with the following mandatory values
      ...
      | (jq).ocs.data.subject.OU                  | (jq) .[1] \|test("^libresign-ca-id:[a-z0-9]{10}(-ca:g[0-9]+-[oc])?$")                      |
